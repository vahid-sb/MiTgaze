...
I changed it so that I can get the data used to generate the heatmap when using kdeplot funciton.
@modified by: Vahid Bokharaie
...
This is simply _distributions module from Seaborn library.
Changed slightly, so kdeplot returns xx, yy and z values used to generate the heatmap.
Apart from that, no other chnages are made.

I forgot which exact version of seaborn was it that I used.
Module was chnaged in November 2019.
@edited by: Vahid Bokharaie
"""

"""Plotting functions for visualizing distributions."""

# from __future__ import division   # gone is Python 2
import numpy as np
...
def _univariate_kdeplot(
    ...
    **kwargs
    ...
    if _has_statsmodels:
        ...
        x, y = _statsmodels_univariate_kde(
            data, kernel, bw, gridsize, cut, clip, cumulative=cumulative
        )
    else:
        ...
        if cumulative:
            raise ImportError(
                "Cumulative distributions are currently"
...
def _statsmodels_univariate_kde(
    data, kernel, bw, gridsize, cut, clip, cumulative=False
):
...
def _bivariate_kdeplot(
    ...
    **kwargs
    ...
    # Calculate the KDE
    if _has_statsmodels:
        xx, yy, z = _statsmodels_bivariate_kde(x, y, bw, gridsize_x, gridsize_y, cut, clip)
    else:
        xx, yy, z = _scipy_bivariate_kde(x, y, bw, gridsize_x, gridsize_y, cut, clip)
...
def _statsmodels_bivariate_kde(x, y, bw, gridsize_x, gridsize_y, cut, clip):
    ...
    x_support = _kde_support(x, kde.bw[0], gridsize_x, cut, clip[0])
    y_support = _kde_support(y, kde.bw[1], gridsize_y, cut, clip[1])
    xx, yy = np.meshgrid(x_support, y_support)
    z = kde.pdf([xx.ravel(), yy.ravel()]).reshape(xx.shape)
    return xx, yy, z


def _scipy_bivariate_kde(x, y, bw, gridsize_x, gridsize_y, cut, clip):
    ...
    x_support = _kde_support(data[:, 0], bw_x, gridsize_x, cut, clip[0])
    y_support = _kde_support(data[:, 1], bw_y, gridsize_y, cut, clip[1])
    xx, yy = np.meshgrid(x_support, y_support)
...
def kdeplot(
    ...
    **kwargs
    ...
    if warn:
        warn_msg = (
            "Passing a 2D dataset for a bivariate plot is deprecated "
...
def rugplot(a, height=0.05, axis="x", ax=None, **kwargs):
    ...
    if vertical:
        ...
        xy_pairs = np.column_stack([np.tile([0, height], len(a)), np.repeat(a, 2)])
    else:
